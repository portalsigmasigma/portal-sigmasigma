// Componente Hero com interatividade de botões
import Link from 'next/link';

interface HeroProps {
  onToggleDisciplinas: () => void;
  disciplinasAbertas: boolean;
}

export default function Hero({ onToggleDisciplinas, disciplinasAbertas }: HeroProps) {
  return (
    <section className="w-full py-10 sm:py-16 md:py-20 flex flex-col items-center justify-center text-center px-4 sm:px-6">
      <div className="max-w-4xl mx-auto flex flex-col items-center space-y-5 sm:space-y-6">
        ...
        {/* Botões de Ação com novo comportamento */}
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 pt-2 w-full sm:w-auto">
          {/* Botão 1: Alterna visibilidade das disciplinas */}
          <button
            onClick={onToggleDisciplinas}
            className="w-full sm:w-auto px-6 py-3.5 sm:py-3 rounded-xl bg-destaque hover:bg-blue-600 text-white font-semibold transition-all shadow-lg shadow-cyan-500/10 text-sm text-center active:scale-95 cursor-pointer"
          >
            {disciplinasAbertas ? 'Ocultar Disciplinas ↑' : 'Explorar Disciplinas ↓'}
          </button>

          {/* Botão 2: Vai direto para o acervo completo de Materiais */}
          <Link
            href="/materiais"
