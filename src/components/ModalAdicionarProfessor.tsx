...
export default function ModalAdicionarProfessor({
  ...
  // Estilo inline para as options no Windows/Chrome
  const optionStyle = {
  ...
  const handleCadastrarProfessor = async (e: React.FormEvent) => {
    ...
    // Tratamento do campo de matéria
    const materiaFinal =
      materiaSelecionada === 'Outra' || !materiaSelecionada
        ? materiaManual.trim() || null
        : materiaSelecionada;
    ...
    const { error } = await supabase.from('professores').insert([
      {
        ...
        sala: sala.trim() || null,
        exigencia: dificuldade || null, // Mapeado para a coluna exigencia
        presenca: passaLista || null,   // Mapeado para a coluna presenca
        dicas: dicas.trim() || null,
        materia: materiaFinal,
        status: 'pendente',             // Para moderação
      },
    ]);
    ...
    if (error) {
      ...
      // Esconde a mensagem e fecha após 2.5s
      setTimeout(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-card border border-borda w-full max-w-lg rounded-2xl p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
        ...
        {/* TELA DE SUCESSO BONITINHA */}
        {mensagemSucesso ? (
          <div className="py-8 px-4 text-center space-y-3 animate-in fade-in zoom-in-95 duration-200">
            <div className="w-12 h-12 bg-green-500/10 border border-green-500/30 rounded-full flex items-center justify-center mx-auto text-green-400 text-xl font-bold">
              ✓
            </div>
            <div className="space-y-1">
              ...
              <p className="text-xs text-texto-secundario max-w-xs mx-auto leading-relaxed">
                As informações do professor foram enviadas para análise e em breve estarão disponíveis na plataforma.
              </p>
            </div>
          </div>
        ) : (
          /* FORMULÁRIO NORMAL */
          <form onSubmit={handleCadastrarProfessor} className="space-y-3">
            {erroMensagem && (
            ...
            {(materiaSelecionada === 'Outra' || materiaSelecionada === '') && (
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="Digite o nome da matéria ou adicione observações..."
                  value={materiaManual}
            ...
            <div>
              ...
              <select
                ...
                className="w-full text-xs p-2.5 rounded-xl bg-fundo border border-borda text-texto-principal focus:border-azul-texto outline-none cursor-pointer"
              >
                <option value="" style={optionStyle}>Não informado</option>
                <option value="Tranquilo" style={optionStyle}>Tranquilo</option>
                <option value="Médio" style={optionStyle}>Médio</option>
                <option value="Exigente" style={optionStyle}>Exigente</option>
              </select>
            </div>
            ...
            <div>
              ...
              >
                <option value="" style={optionStyle}>Não informado</option>
                <option value="Passa Lista" style={optionStyle}>Passa Lista</option>
                <option value="Sempre" style={optionStyle}>Sempre</option>
                <option value="Às vezes" style={optionStyle}>Às vezes</option>
                <option value="Raramente" style={optionStyle}>Raramente</option>
                <option value="Não cobra" style={optionStyle}>Não cobra</option>
              </select>
            </div>
